...
def resize_image(uploaded_image, target_width):
    try:
...
# Let users input their desired width
target_width = st.slider("Choose the target width for resizing:", min_value=100, max_value=4000, value=1920, step=10)
...
if uploaded_images:
    ...
    with zipfile.ZipFile(zip_buffer, 'a', zipfile.ZIP_DEFLATED) as zf:
        for uploaded_image in uploaded_images:
            try:
                resized_img = resize_image(uploaded_image, target_width)
